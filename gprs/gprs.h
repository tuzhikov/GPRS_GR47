...
#define SIZE_BUF_HDR_GPRS      30     //макс. длина заголовка HTTP (напр., "HTTP/1.1 100 Continue")
#define TIMEOUT_HTTP_STREAM     8     //[*1s]
#define TIMEOUT_HTTP_REQ      120     //[*1s]

#define INPAC_HEADER_LEN        5
#define INPAC_HEADER_STR   "KOD8#"    //заголовок пакета Navigator DC в пакете HTTP
...
typedef union GPRSGL {
    ...
    struct {                               //Flags:
        unsigned int abs_off /*0001*/ : 1; // "GPRS не используется (работа только по SMS)" (v)
        unsigned int f_cycle /*0002*/ : 1; // "Полный цикл опроса модема" (m)
        unsigned int gprs_md /*0004*/ : 1; // "Режим GPRS активен" (m)
        unsigned int command /*0008*/ : 1; // "Вход/выход в режим AT-команд" (v/m)
        unsigned int ret_cmd /*0010*/ : 1; // "Возврат в режим AT-команд" (m)
        unsigned int t_input /*0020*/ : 1; // "Разрыв входного потока" (m)
        unsigned int gprs_er /*0040*/ : 1; //
        unsigned int new_req /*0080*/ : 1; // "Получен новый запрос от ЦДП" (m)
        unsigned int         /*0100*/ : 1; //
