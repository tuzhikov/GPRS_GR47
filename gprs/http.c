...
void automat_http()
{
    ...
    switch (StateHTTP)
    {
        case 0x01: //send HTTP header
            ...
            {
                if (GSM_CTS) StSYSTEM |= 0x3000; //"HTTP, передача данных (CTS busy)"
                break;
            }
            StSYSTEM &= ~0x7000; //"HTTP, Idle"
            //* есть данные для отправки (IOStreamBuf.Out[]/.OutAuto[])
            //IOStreamState.Out |= 0x8800;
			if(!gprs.b.sending){
				//передаем по HTTP
				if((IOStreamState.Out==0x0800)&&(continueSession==retOk)){//данных много надо передавать
					IOStreamState.Out = 0xe583;
					}
				//
			 if((IOStreamState.Out & 0xC300) == 0xC100)
            	{
                StSYSTEM |= 0x1000; //"HTTP, отправка данных"
        ...
        case 0x05: //send HTTP content
            if (gprs.b.ackhttp && !gprs.b.sending || !timer_http_wait)
            {
				...
				//sendStatusGPRS=retOk; // это блок передали, надо пеердать следующий
				StateHTTP = 0x10;
            }
        break;
        ...
        case 0x10: //send all
            ...
            {
                StSYSTEM &= ~0x1000;
				sendStatusGPRS = retOk; // передали пакет данных по http, разрешаем опрос детектора
				if (!timer_http_delay) cntPacInFrame = 0;
                timer_http_delay = 100;//250; //[*0.01s]
                if (++cntPacInFrame >= 3)
                {
                    StSYSTEM |= 0x2000; //"HTTP, передача данных (send buffer)"
                    cntPacInFrame = 0;
                    StateHTTP = 0x15;
                }
                else
...
// сохраняем время выборки данных
        ...
        switch (g)
        {
            ...
            case 2:
                for (i=0; i<http_header[g].len; i++)
                {
                    if (http_header[g].str[i] == '#')
                    {
                        if ((IOStreamBuf.Header.id == 0x32) || //* авторежим KOD8
                            (IOStreamBuf.Header.id >= 0x4D) && (IOStreamBuf.Header.id <= 0x51)) //* запрос KOD8
                        {
                            //48 - '\0' + текст;
                            //context_gsm.simid_len - длина ID SIM-карты;
                            //8 - время (4+4);
                            //10 - поле Data[];
                            //4 - индекс длины поля датчиков (2+2);
                            //8 - поле Port[]
                            ...
                            if (pChar[2] & 0x01) Char += 54; //т.е. есть данные с датчика 0
                            if (pChar[2] & 0x02) Char += 54; //т.е. есть данные с датчика 1
                            L += Hex_to_DecASCII_5(Char, &buf_http[L]);
                        }
                        else //* все остальные режимы
                        {
                            //индекс длины поля Text[]
                            ...
                            //12 - '\0' + текст;
                            //context_gsm.simid_len - длина ID SIM-карты;
                            //cnt - [SMS Packet Header (5)] + длина поля Text[]
...
unsigned int formingHTTPContent()
{
  ...
  unsigned char xdata cnt_cr;
    ...
    {g = 0; S = 5;}
        ...
        else {g = 8; S = 9;} //* все остальные режимы
    //*--------------*
    if ((IOStreamState.Out & 0x3800) == 0x0800)
        pChar = &IOStreamBuf.OutAuto;
    else
        ...
        switch (g)
        {
            ...
            case 1:
                for (i=0; i<http_body[g].len; i++)
                {
                    ...
                    {
                        int_timer2_no;
                        memcpy(&buf_http[L], &Timer_inside, 4); //время отправки
                        int_timer2_yes;
                        L += 4;
                        memcpy(&buf_http[L], &pChar[9], 4); //время
                        L += 4;
                    }
                    else
                        buf_http[L++] = http_body[g].str[i];
            ...
            case 2:
                for (i=0; i<http_body[g].len; i++)
                {
                    if (http_body[g].str[i] == '#')
                    {
                        memcpy(&buf_http[L], &pChar[13], 10); //все данные (кроме датчиков трансп.)
                        L += 10;
                    }
                    else
                        buf_http[L++] = http_body[g].str[i];
            ...
            case 4:
                ...
                for (i=0; i<http_body[g].len; i++)
                {
                    if (http_body[g].str[i] == '#')
                    {
                        ...
                        {
                            case 0: //Len=f(Event)
                                if (pChar[2] & (1<<cnt)) //т.е. есть данные с датчика 0/1
                                {
                                    buf_http[L++] = '5';
                                    buf_http[L++] = '4';
                                }
            ...
            case 5:
                for (i=0; i<http_body[g].len; i++)
                {
                    if (http_body[g].str[i] == '#')
                    {
                        memcpy(&buf_http[L], &LOutK8, 4); //счетчик отправленных байт в KOD8
                        L += 4;
                        int_uart1_no;
                        memcpy(&buf_http[L], &LInK8, 4); //счетчик принятых байт из KOD8
                        int_uart1_yes;
                        L += 4;
                    }
                    else
    ...
    //добавляем '\0' для исключения \x0D, \x0A
    buf_http[L++] = '\0';
