...
unsigned char xdata OutMAXPointer1 = 0; //ук. на читаемый из буфера байт
unsigned char xdata OutMAXPointer2 = 0; //ук. на записываемый в буфер байт
bit _flag_tangenta = 0; //"0" - передача, "1" - прием


/*------------------------------- Instal UART0, UARt1 ------------------------------*/
void Init_uart0 (void){FlagTX0 = yes;}
void Init_uart1 (void){FlagTX1 = yes;}
/*---------------------------- Отправка UART0, UARt1--------------------------------*/
...
{
unsigned short  StrLength = strlen(str); //длинна сторки в байтах без терминального нуля
...
{
unsigned char  StrLength = strlen(str); //длинна сторки в байтах без терминального нуля
unsigned short index = 0;
...
/*------------------ Очистить буффер для UART 0? UARt1 ---------------------------- */
...
void HoldSPI_MAX() //ожидание освобождения и "захват" шины SPI (for MAX3100 UART)
{
...
void FreeSPI_MAX() //освободить SPI и восстановить параметры (for MAX3100 UART)
{
    CSEL0 = 1;
    SFRPAGE = SPI0_PAGE;
...
{
   _flag_tangenta=0;
    pcontext[0] = 0x43; // Посылаем 'C' и устанавливаем RTS в 1 (передача данных).
    OutMAXPointer2 = 1;
}
...
{
   _flag_tangenta=1;
    pcontext[0] = 0x43; // Посылаем 'C' и устанавливаем RTS в 0 (прием данных).
    OutMAXPointer2 = 1;
}
